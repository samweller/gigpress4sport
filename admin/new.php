...
function gigpress_add() {
  ...
  <?php
    ...
    if(is_array($result)) {
      ...
      $show_opponent_id = $_POST['show_opponent_id'];
      $show_venue_id = $_POST['show_venue_id'];
      $new_artist_name = gigpress_db_out($_POST['artist_name']);
      $new_opponent_name = gigpress_db_out($_POST['opponent_name']);
      ...
      ");
        if($show_edit) {
          ...
          foreach($show_edit as $show) {
            ...
            $show_opponent_id = $show->show_opponent_id;
    ...
    <?php } ?>
      ...
      <tbody>
        ...
        <tr>
          ...
          <!-- Testing new bit -->
          <tr>
            <th scope="row"><label for="show_opponent_id"><?php _e("Opponent", "gigpress") ?>:<span class="gp-required">*</span></label></th>
            <td>
            <select name="show_opponent_id" id="show_opponent_id" class="can-add-new">
                <?php $artists = fetch_gigpress_artists();
                if($artists != FALSE) {
                  foreach($artists as $artist) {
                    $artist_name = gigpress_db_out($artist->artist_name);
                    echo("<option value=\"$artist->artist_id\"");
                    if($show_opponent_id == $artist->artist_id) echo(' selected="selected"');
                    echo(">$artist_name</option>\n\t\t\t");
                  }
                } else {
                  echo('<option value="0" selected="selected">' . __("No artists in the database", "gigpress") . '</option>');
      ...
      <tr>
          ...
          <td>
            <select name="show_related" id="show_related" class="can-add-new">
              ...
              <?php 
              $entries = $wpdb->get_results("SELECT ID, post_title FROM " . $wpdb->prefix . "posts WHERE (post_status = 'publish' OR post_status = 'future') AND post_type = 'post' ORDER BY ID DESC LIMIT 500", ARRAY_A);  
              if($entries != FALSE) {           
