...
function gigpress_admin_shows() {
  ...
  if(isset($_GET['opponent_id']) && $_GET['opponent_id'] != '-1') {
    $further_where .= ' AND s.show_opponent_id = ' . $wpdb->prepare('%d', $_GET['opponent_id']) . ' ';
    $pagination_args['opponent_id'] = $_GET['opponent_id'];
    $url_args .= '&amp;opponent_id=' . $_GET['opponent_id'];
  }
  ...
  <div class="wrap gigpress">
      ...
      <div class="alignleft">
        <form action="" method="get">
          <div>
            ...
            if($artistdata) {
              foreach($artistdata as $artist) {
                $selected = (isset($_GET['opponent_id']) && $_GET['opponent_id'] == $artist->opponent_id) ? ' selected="selected"' : '';
                echo('<option value="' . $artist->opponent_id . '"' . $selected . '>' . gigpress_db_out($artist->opponent_id) . '</option>');
              }
            } else {
              echo('<option value="-1">' . __("No artists in the database", "gigpress") . '</option>');
            }
    ...
    <table class="widefat">
      <thead>
        <tr>
          ...
          <th scope="col"><?php _e("Home Team", "gigpress"); ?></th>
          <th scope="col"><?php _e("Away Team", "gigpress"); ?></th>
    ...
    if($shows != FALSE) {
      ...
      foreach($shows as $show) {
        ...
        <tr class="<?php echo 'gigpress-' . $showdata['status']; ?>">
          ...
          <td><?php echo $showdata['opponent']; ?></td>
    ...
    ?>
      <tr><td colspan="9"><?php _e("Sorry, no shows to display based on your criteria.", "gigpress"); ?></td></tr>
    <?php } ?>
      </tbody>
    </table>
    <div class="tablenav">
